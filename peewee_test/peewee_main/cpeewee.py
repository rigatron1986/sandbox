...
from .monitoring import CallingCountMonitoring
from .qt_timer_handler import add_connection

ignore_sql_log = os.environ.get('BFX_IGNORE_SQL_LOG')


class PeeweeMonitoring(CallingCountMonitoring):
    def __init__(self, name=None):
        super(PeeweeMonitoring, self).__init__(name)

        class MonitoringDecorator(self.decorator):
            def __init__(self, name=None):
                super(MonitoringDecorator, self).__init__(name)
                self.database_access_history = []

        self.decorator = MonitoringDecorator

        def before_execute_sql(decorator, func, args, kwargs):
            if not len(args):
                return
            db_instance = args[0]
            if not isinstance(db_instance, Database):
                return
            if isinstance(db_instance, MySQLDatabase):
                db_name = "MySQL"
            elif isinstance(db_instance, PostgresqlDatabase):
                db_name = "Postgresql"
            else:
                db_name = "Others"
            decorator.database_access_history.append((db_name, time.time()))

        self.decorator.register_before_func_running_callback("database_accessing", before_execute_sql)

        self.decorator.register_statistics_method(
            "mysql_query_request_count",
            lambda x: len(filter(lambda y: y[0] == "MySQL", x.database_access_history))
        )
        self.decorator.register_statistics_method(
            "postgresql_query_request_count",
            lambda x: len(filter(lambda y: y[0] == "Postgresql", x.database_access_history))
        )
        self.decorator.register_statistics_method(
            "other_query_request_count",
            lambda x: len(filter(lambda y: y[0] == "Others", x.database_access_history))
        )


calling_count_monitoring = PeeweeMonitoring(__name__)

# Because we will fetch all the rows after any execute
# So we have this param to set what's the biggest query
# If your query result is bigger than it
# we will raise a ResultTooBig error
...
class Node(object):
    ...
    def not_like(self, rhs):
        return Expression(self, OP.NOT_LIKE, rhs)
...
class FieldDescriptor(object):
    ...
    def __set__(self, instance, value):
        if isinstance(self.field, CharField) and isinstance(value, basestring) and len(value) > self.field.max_length:
            raise CharValueError(
                instance._meta.database.database,
                instance._meta.db_table,
                self.field.db_column,
...
class ForeignKeyField(IntegerField):
    ...
    def _get_related_name(self):
        ...
        elif self._related_name:
            return self._related_name
        elif self.model_class:
            return ('%s_set' % self.model_class._meta.name)
        else:
            return None
    ...
    def add_to_class(self, model_class, name):
        ...
        if model_class._meta.validate_backrefs:
            if not model_class._meta.db_table.endswith('_history'):
...
class QueryCompiler(object):
    ...
    def _parse_entity(self, node, alias_map, conv):
        quote = self.quote
        path = node.path
        entity_map = map(quote, path)
        joined_map = '.'.join(entity_map)
        return joined_map, []
...
class QueryResultCursor(object):
    """
    Fetch all the rows from source cursor to memory

    This class only support few method which is used in QueryResultWrapper.
    When a new instance is generated, it will call `fetchall` to fetch all the rows to memory
    Then every fetch operation will happened without source cursor

    close: same as source cursor
    description: same as source cursor
    name: same as source cursor
    fetchone: fetchone result from memory
    """
    ...
    def __init__(self, cursor):
        ...
        # Check the rows length, if it was longer than the setting
        # Raise the ResultTooBig error
        if self.source_cursor.rowcount > GlobalResultLimit:
            raise ResultTooBig('The query result is more than {} rows, '
                               'please optimize your query!'.format(GlobalResultLimit))

        self.rows = self.source_cursor.fetchall()
        self.row_iterator = iter(self.rows)
        self.close = self.source_cursor.close
        self.description = self.source_cursor.description
        self.name = getattr(self.source_cursor, 'name', None)
...
class Query(Node):
    ...
    def convert_dict_to_node(self, qdict):
        from ..base.core import EntityForeignKeyField
        accum = []
        joins = []
        ...
        for key, value in sorted(qdict.items()):
            ...
            for piece in key.split('__'):
                ...
                if isinstance(model_attr, relationship):
                    if isinstance(model_attr, EntityForeignKeyField):
                        model_attr.rel_model = value.__class__
...
class SelectQuery(Query):
        ...
        try:
            ...
            try:
                ...
                raise self.model_class.ExistsTooMany(
                    'Instance matching query exists too many:\nSQL: %s\nPARAMS: %s'
                    % self.sql())
    ...
    def _execute(self):
        # If the query don't have a limit, we will force add it.
        if not self._limit:
            # To help the boundary detection, so the limit should be greater than the global setting
            self._limit = GlobalResultLimit + 1
        return super(SelectQuery, self)._execute()
...
class CharValueError(PeeweeException):
    def __init__(self, database, table, field, length):
        super(CharValueError, self).__init__("String is too long, {0}.{1}.{2} just support {3} characters!".format(
...
class ExceptionWrapper(object):
    ...
    def get_new_value(self, new_type, old_value):
        if PY26:
            exc_args = old_value
        else:
            exc_args = old_value.args
        new_value = new_type(*exc_args)
        new_value.err_type = -1
        new_value.err_msg = ''
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if exc_type.__name__ in self.exceptions:
            ...
            reraise(new_type, self.get_new_value(new_type, exc_value), traceback)


class MySQLExceptionWrapper(ExceptionWrapper):
    def get_new_value(self, new_type, old_value):
        new_value = super(MySQLExceptionWrapper, self).get_new_value(new_type, old_value)
        try:
            new_value.err_type = old_value.args[0]
            new_value.err_msg = old_value.args[1]
        finally:
            return new_value


class PostgresqlExceptionWrapper(ExceptionWrapper):
    def get_new_value(self, new_type, old_value):
        new_value = super(PostgresqlExceptionWrapper, self).get_new_value(new_type, old_value)
        try:
            new_value.err_type = old_value.pgcode
            new_value.err_msg = old_value.pgerror
        finally:
            return new_value
...
class Database(object):
    ...
    exception_wrapper_class = ExceptionWrapper

    connection_error_codes = set()
    ...
    def exception_wrapper(self):
        return self.exception_wrapper_class(self.exceptions)
        ...
        with self._conn_lock:
            ...
            with self.exception_wrapper():
                ...
                self._local.conn = self._connect(
                    ...
                    **self.connect_kwargs)
                add_connection(self)
                logger.debug("Connect to host: {} db: {} in {:.3f}".format(
                    self.connect_kwargs.get('host'), self.database,
                    (time.time() - con_time) * 1000))
                db_conn.append((self._local, datetime.datetime.now()))
    ...
    def close(self):
        try:
            logger.debug('host: {} db: {} connection was closed.\n{}'.format(
                self.connect_kwargs.get('host'), self.database,
    ...
    @calling_count_monitoring.decorator("execute_sql")
    def execute_sql(self, sql, params=None, require_commit=True):
        ...
        while True:
            ...
            try:
                ...
                logger.error('A peewee exception has occurred: err_code: {} err_msg: {}'.format(
                    e.err_type, e))
                logger.error(traceback.format_exc())
                logger.error('host: {} db: {} query: {} params: {} failed in: {:.3f}'.format(
                    self.connect_kwargs.get('host'), self.database,
                    repr(sql), params,
                    (time.time() - exc_time) * 1000)
                )
                wait_time *= 2
                retry_times -= 1

                if e.err_type not in self.connection_error_codes:
                    raise
                if retry_times <= 0:
                    raise
                logger.warning("Retry to connect database after {} seconds.".format(wait_time))
                time.sleep(wait_time)
                self.init(database=self.database, **self.connect_kwargs)
            else:
                if ignore_sql_log is None:
                    logger.debug('host: {} db: {} query: {} params: {} succeed in: {:.3f}'.format(
                        self.connect_kwargs.get('host'), self.database,
                        repr(sql), params,
                        (time.time() - exc_time) * 1000))
                return cursor
    ...
    def create_history_table(self, model_class, safe=False):
        qc = self.compiler()
        model_class.create_history_model()
        return self.execute_sql(*qc.create_table(model_class.history_model, safe))
...
class PostgresqlDatabase(Database):
    ...
    exception_wrapper_class = PostgresqlExceptionWrapper
    ...
    def _connect(self, database, encoding=None, **kwargs):
        ...
        conn_kwargs = {'connect_timeout': 10, 'options': '-c statement_timeout=300s'}
        conn_kwargs.update(kwargs)
        conn = psycopg2.connect(database=database, **conn_kwargs)
...
class MySQLDatabase(Database):
    ...
    exception_wrapper_class = MySQLExceptionWrapper
    ...
    def _connect(self, database, **kwargs):
        ...
        s = socket.socket()
        try:
            # Try to connect to remote db server
            # Then get current host ip
            s.connect((conn_kwargs['host'], conn_kwargs['port']))
            current_host = s.getsockname()[0]
        except:
            # If some exceptions happened unexpected
            # Get current host ip by fqdn.
            current_host = socket.gethostbyname(socket.getfqdn())
        finally:
            s.close()

        # Connect to database
        conn = mysql.connect(db=database, **conn_kwargs)

        # Set variables for this connection
        cur = conn.cursor()
        try:
            cur.execute("SET @BFX_CURRENT_USER=%s", (getpass.getuser(),))
            cur.execute("SET @BFX_CURRENT_HOST=%s", (current_host,))
        except mysql.Error:
            pass
        finally:
            cur.close()
...
class ExistsTooMany(Exception): pass


class ResultTooBig(Exception): pass
...
class ModelOptions(object):
    ...
    def rel_for_model(self, model, field_obj=None, multi=False):
        from .core import EntityForeignKeyField
        ...
        for field in self.sorted_fields:
            is_rel_model = False
            if isinstance(field, EntityForeignKeyField) and model in field.rel_models:
                field.rel_model = model
                is_rel_model = True
            elif isinstance(field, ForeignKeyField) and field.rel_model == model:
                is_rel_model = True
            if is_rel_model:
                is_match = (
...
class BaseModel(type):
    inheritable = set([
        'constraints', 'database', 'db_table_func', 'indexes', 'order_by', 'db_table',
    ...
    def __new__(cls, name, bases, attrs):
        ...
        meta_options = {}
        meta_options["manual_attrs"] = []
        manual_attrs = meta_options["manual_attrs"]
        ...
        for b in bases:
            ...
            base_manual_attrs = base_meta.__dict__.get("manual_attrs")
            for (k, v) in base_meta.__dict__.items():
                if k in all_inheritable and (k not in meta_options or k in base_manual_attrs):
                    meta_options[k] = v
                if k == "manual_attrs":
                    manual_attrs += v
        ...
        # If the db_table has not been explicitly specified, derive it from the class name
        if not cls._meta.db_table or "db_table" not in manual_attrs:
        ...
        exc_name = '%sExistsTooMany' % cls.__name__
        exc_attrs = {'__module__': cls.__module__}
        exception_class = type(exc_name, (ExistsTooMany,), exc_attrs)
        cls.ExistsTooMany = exception_class
        cls._meta.prepared()

        if hasattr(cls, 'validate_model'):
            cls.validate_model()

        cls.module_context = importlib.import_module(cls.__module__)
...
class Model(with_metaclass(BaseModel)):
    ...
    @classmethod
    def get_unique_row(cls, *query, **kwargs):
        sq = cls.select().naive()
        if query:
            sq = sq.where(*query)
        if kwargs:
            sq = sq.filter(**kwargs)
        return sq.get_unique_row()
    ...
    @classmethod
    def history_table_exists(cls):
        kwargs = {}
        cls.history_table = '{}_{}'.format(
            cls._meta.db_table, 'history'
        )
        return cls.history_table in cls._meta.database.get_tables(**kwargs)

    @classmethod
    def create_table(cls, fail_silently=False, generate_sql=True):
    ...
    @classmethod
    def create_history_model(cls):
        attrs = {}
        for field in cls._meta.sorted_fields:
            new_key = field.db_column
            new_value = deepcopy(field)

            if isinstance(new_value, PrimaryKeyField):
                new_value = IntegerField(primary_key=False)
            if isinstance(new_value, ForeignKeyField):
                new_value = IntegerField(primary_key=False)

            if isinstance(new_value, IntegerField):
                new_value.index = True

            if new_value.primary_key:
                new_value.primary_key = False

            if new_value.unique:
                new_value.unique = False
                new_value.index = True
            if new_key == 'id':
                new_key = '{table_name}_{ref_id}'.format(
        ...
        attrs.update({
            'id': PrimaryKeyField(),
            'updated_by': CharField(index=True, max_length=16),
            'ip_address': CharField(index=True, max_length=64),
            'user_action': CharField(index=True, max_length=16),
            'updated_at': DateTimeField(
                constraints=[SQL('DEFAULT CURRENT_TIMESTAMP')], index=True)
        })

        new_indexes = deepcopy(cls._meta.indexes)

        for _index in new_indexes:
            if _index[1]:
                for field in cls._meta.sorted_fields:
                    if field.name in _index[0] and field.db_column not in _index[0]:
                        inx = _index[0].index(field.name)
                        _index[0][inx] = field.db_column

                new_index = (_index[0], False)
                new_indexes.remove(_index)
                new_indexes.append(new_index)

        if isinstance(cls._meta.primary_key, CompositeKey):
            modified_index = (list(cls._meta.primary_key.field_names), False)
            if modified_index not in new_indexes:
                new_indexes.append(modified_index)

        class Meta:
            database = cls._meta.database
            db_table = '{model}_history'.format(
                model=cls._meta.db_table,
            )
            primary_key = False
            indexes = new_indexes
            validate_backrefs = cls._meta.validate_backrefs

        attrs['Meta'] = Meta

        cls.history_model = type(
            '{}History'.format(
                cls.__name__
            ),
            (Model,),
    ...
    @classmethod
    def create_history_table(cls, fail_silently=False, generate_sql=True):
        if fail_silently and cls.history_table_exists():
            return

        db = cls._meta.database
        db.create_history_table(cls)
        cls.history_model._create_indexes()
        if generate_sql:
            save_sql(cls.history_model)

    @classmethod
    def enable_history(cls, safe=False, generate_sql=True):
        cls.create_history_table(fail_silently=safe, generate_sql=generate_sql)
    ...
    def as_entity(cls):
        ...
        elif isinstance(cls._meta.database, MySQLDatabase):
            return Entity(cls._meta.database.database, cls._meta.db_table)
        else:
...
db_conn = []
db_conn_context = False
db_context_depth = 0


def close_conn_on_threshold(conn, timeout):
    threshold_mins = timeout
    now = datetime.datetime.now()
    elasped_time = now - conn[1]
    mins, secs = divmod(elasped_time.total_seconds(), 60)

    if mins >= threshold_mins and not conn[0].closed:
        conn[0].conn.close()
        conn[0].closed = True
        db_conn.remove(conn)
...
def db_connection(timeout=10):
    ...
    All database operations are allowed only if it is wrapped with in this context.
    It checks weather any connection is open for more than the allowed threshold time
    and re-cycles it.
    :param timeout: threshold timeout in minutes.
    """

    global db_conn, db_conn_context, db_context_depth

    db_conn_context = True
    db_context_depth += 1

    for conn in db_conn:
        close_conn_on_threshold(conn, timeout=timeout)

    yield

    db_context_depth -= 1
    if db_context_depth == 0:
        db_conn_context = False


def save_sql(model):
    """
    Hook method to save the sql right after a table is created. This method will be called
    internally once create_table or enable_history commands are executed. The generated sql
    file will be saved under /sw/ple/workspace for now and the file name would look like
    bfx_<database_name>_<table_name>_00001.sql.

    The version number is by default 00001 because create table should generally be called
    once and it will be first version on your table. After that, any changes to the table
    should come via peewee.migration module. This was all our schema changes will be tracked
    ...
    try:
        ...
        with FileHandler(environment=model._meta.database.database,
                            file_suffix=model._meta.db_table) as _file_handler:

            schema_file = _file_handler.get()

            schema_file.write('{} {}\n'.format(delim * 2, delim * 76))
            schema_file.write('{} {}\n'.format(delim * 2, 'INITIAL SCHEMA MIGRATION'))
            schema_file.write('{} {}\n'.format(delim * 2, delim * 76))

            for query in model.sqlall():
                schema_file.write("{};\n".format(query))

            schema_file.write('{} {}\n'.format(delim * 2, delim * 76))
            schema_file.write('{} {}\n'.format(delim * 2, 'CREATED BY: {}'.format(username)))
            schema_file.write('{} {}\n'.format(delim * 2, 'FROM: {}'.format(hostname)))
            schema_file.write('{} {}\n'.format(
                delim * 2, 'DATE: {}'.format(str(datetime.datetime.now())))
            )
            schema_file.write('{} {}\n\n\n'.format(delim * 2, delim * 76))
