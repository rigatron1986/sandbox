...
from cpeewee import *
from cpeewee import binary_construct
from cpeewee import Field
from cpeewee import FieldDescriptor
from cpeewee import SelectQuery
from cpeewee import unicode_type
...
class ManyToManyField(Field):
    def __init__(self, rel_model, related_name=None, through_model=None,
                 _is_backref=False, _is_internal=False, *args, **kwargs):
    ...
    def clone_base(self, **kwargs):
        return super(ManyToManyField, self).clone_base(
            rel_model=self.rel_model,
            related_name=self._related_name,
            through_model=self._through_model,
            _is_backref=self._is_backref,
            **kwargs)
    ...
    def add_to_class(self, model_class, name):
        ...
        if not self._is_backref:
            backref = ManyToManyField(
                self.model_class,
                through_model=self._through_model,
                _is_backref=True, _is_internal=True)
    ...
    def get_through_model(self):
        if not self._through_model:
            ...
            lhs_field_name = lhs._meta.name
            rhs_field_name = rhs._meta.name

            if lhs == rhs:
                lhs_field_name = 'from_{}'.format(lhs_field_name)
                rhs_field_name = 'to_{}'.format(rhs_field_name)

            class Meta:
                database = self.model_class._meta.database
                db_table = '{lhs_table}_{rhs_table}_through'.format(
                    lhs_table=lhs._meta.db_table,
                    rhs_table=rhs._meta.db_table,
                )
                indexes = (
            ...
            attrs = {
                lhs_field_name: ForeignKeyField(rel_model=lhs),
                rhs_field_name: ForeignKeyField(rel_model=rhs)}
            attrs['Meta'] = Meta

            self._through_model = type(
                '{lhs_name}{rhs_name}Through'.format(
                    lhs_name = lhs.__name__,
...
class ManyToManyFieldDescriptor(FieldDescriptor):
    def __init__(self, field):
        ...
        self._is_internal = field._is_internal
        ...
        if self.src_fk.name == self.dest_fk.name:
            self.src_fk, self.dest_fk = self.through_model._meta.rel_for_model(
                self.rel_model, multi=True)
            if self._is_internal:
                self.src_fk, self.dest_fk = self.dest_fk, self.src_fk
    ...
    def __get__(self, instance, instance_type=None):
        if instance is not None:
            ...
            if self.rel_model == self.model_class:
                model_class = self.model_class.alias()
                from_field, to_field = [
                    _field for _field in self.through_model._meta.sorted_fields
                    if isinstance(_field, ForeignKeyField)
                ]
                rel_model_join_on = to_field
                model_class_join_on = (from_field == model_class.id)
                if self._is_internal:
                    rel_model_join_on = from_field
                    model_class_join_on = (to_field == model_class.id)

            query = (ManyToManyQuery(instance, self, self.rel_model)
                     .select()
                     .join(self.through_model, on=rel_model_join_on)
                     .join(model_class, on=model_class_join_on)
                     .where(self.src_fk == instance)
                     .switch(self.rel_model))

            # Hack to get ManyToManyQueries to ignore retired entities in shotgun
            try:
                query = query.where(self.rel_model.date_retired == None)
